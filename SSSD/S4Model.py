...
def cauchy_conj(v, z, w):
    ...
    cauchy_mult = Genred(
        ...
        reduction_op='Sum',
        axis=1,
        dtype='float32' if v.dtype == tf.complext else 'float64',
    )
...
def cauchy_slow(v, z, w):
    ...
    cauchy_matrix = v/ (z - w) # (... N L)
    return tf.math.sum(cauchy_matrix, axis=-2)
            
    
        
def _c2r(x):
    y1 = tf.expand_dims(tf.math.real(x), -1)
    y2 = tf.expand_dims(tf.math.imag(x), -1)
    res = tf.concat([y1, y2], -1)
    return res
_r2c = tf.complex
...
def Activation(activation=None, dim=-1):
    if activation in [ None, 'id', 'identity', 'linear' ]:
        ...
        return tf.keras.activations.silu()
...
class TransposedLinear(tf.keras.layers.Layer):
    ...
    def __init__(self, d_input, d_output, bias=True):
        ...
        self.weight = tf.Variable(tf.random.normal([d_output, d_input]),trainable = True)

        if bias:
            bound = 1 / math.sqrt(d_input)
            self.bias = tf.Variable(tf.random.uniform(minval = -bound, maxcal = bound, shape=[d_output, 1]))
        else:
            self.bias = 0.0
...
def nplr(measure, N, rank=1, dtype=tf.float32):
    ...
    AP = A + tf.math.sum(tf.expand_dims(P,-2)*tf.expand_dims(P,-1), axis=-3)
...
class SSKernelNPLR(tf.keras.layers.Layer):
    ...
    def _omega(self, L, dtype, device, cache=True):
        ...
        omega = omega ** tf.range(0, L // 2 + 1, device=device)
        z = 2 * (1 - omega) / (1 + omega)
        if cache:
    ...
    def __init__(
        ...
        C = C.expand(tf.broadcast_static_shape(C.shape, [1, self.H, self.N])) # (H, C, N)
    ...
    def _setup_linear(self):
        ...
        D = (2.0 / tf.expand_dims(dt, -1) - w).reciprocal()  # (H, N)
        R = (tf.eye(self.rank, dtype=w.dtype, device=w.device) + 2*contract('r h n, h n, s h n -> h r s', Q, D, P).real) # (H r r)
    ...
    def setup_step(self, mode='dense'):
        ...
        dA_L = power(self.L, self.dA)
        I = tf.eye(self.dA.size(-1)).to(dA_L)
...
class HippoSSKernel(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        cdtype = tf.complex if dtype == tf.float else tf.complex
        self.rate = None if resample else 1.0
        self.channels = channels

        # Generate dt
        log_dt = tf.random.normal(self.H, dtype=dtype) * (
...
def get_torch_trans(heads=8, layers=1, channels=64):
    ...
    return Encoder(encoder_layer, num_layers=layers)
...
class S4(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        dropout_fn = tf.keras.layers.Dropout if self.transposed else tf.keras.layers.Dropout
...
class S4Layer(nn.Module):
    ...
    def __init__(self, features, lmax, N=64, dropout=0.0, bidirectional=True, layer_norm=True):
        ...
        self.norm_layer = tf.keras.layers.LayerNormalization(features) if layer_norm else tf.identity() 
        self.dropout = tf.keras.layers.Dropout(dropout) if dropout>0 else tf.identity()
